...
\chapter{Analyse}
	...
	\begin{itemize} % liste entrée menu & in_game
		...
		\begin{itemize} % liste choix in_game
			...
			\item Soit il s'agit d'une commande inGame. Par exemple, si l'utilisateur veut quitter, sauvegarder, annuler le dernier coup effectué, ou bien afficher l'aide des commandes. En fonctions de la valeurs reçue, le moteur retourne des valeurs spécifiques de GAGNANT (int retourné par le moteur) qui seront interprétées par le main.\\
	
	L'indépendance du MOTEUR est donc l'un des axes principaux sur lequel T.H. à travaillé.\\
		
        %- jeu contre IA
        %- jeu à 6 joueurs
        \paragraph*{} % sauvegarde
	En ce qui concerne la sauvegarde, rien de vraiment spéciale. L'utilisateur ne peut sauvegarder qu'une partie en cours d'éxécution. Il s'agit donc d'un commande inGame, c'est-à-dire que le moteur retour cette valeur au main, qui lancera lemodule de demande de slot à l'utilisateur. 
